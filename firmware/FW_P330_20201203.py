...
HeartBeat = {'Type': 'HeartBeat',
             ...
             'ShutterTime':5,
             'Distance':100,
             'rtmp_url':'rtmp://push.uavlive.cn/ccssoft/test'
             }
...
def read_mavlink():  # 读mavlink数据放入心跳json包
    ...
    while True:
        try:
            ...
            if 'HEARTBEAT' in the_connect.messages:
            ...
            elif 'SYS_STATUS' in the_connect.messages:
            ...
            elif 'VFR_HUD' in the_connect.messages:
            ...
            elif 'ATTITUDE' in the_connect.messages:
            ...
            elif 'GLOBAL_POSITION_INT' in the_connect.messages:
            ...
            elif 'COMMAND_ACK' in the_connect.messages:
                ...
                ack=result
            elif 'MISSION_COUNT' in the_connect.messages:
                read_plan_count = the_connect.messages['MISSION_COUNT'].count
                ack=0
            elif 'MISSION_ITEM_INT' in the_connect.messages:
                read_dict={}
            ...
            elif 'MISSION_REQUEST_INT' in the_connect.messages:
                seq = the_connect.messages['MISSION_REQUEST_INT'].seq
                ack = 0
            elif 'MISSION_REQUEST' in the_connect.messages:
                seq = the_connect.messages['MISSION_REQUEST'].seq
                ack = 0
            elif 'MISSION_ACK' in the_connect.messages:
                seq = the_connect.messages['MISSION_ACK'].type
                ack = 0
            elif 'GPS2_RAW' in the_connect.messages:
            ...
            elif 'PARAM_VALUE' in the_connect.messages:
                ack=0
                # logger.info('PARAM_VALUE')
                # logger.info(the_connect.messages['PARAM_VALUE'].param_id)
                # logger.info(the_connect.messages['PARAM_VALUE'].param_value)
                # logger.info(the_connect.messages['PARAM_VALUE'].param_type)
            # elif 'PARAM_ACK' in the_connect.messages:
            #     seq = the_connect.messages['PARAM_ACK'].type
            #     logger.info("PARAM_ACK")
            #     ack = 0
            else:
                pass
                # logger.info(the_connect.messages)
    ...
    while True:
        ...
        if os.path.exists("/dev/sda1"):#插入了U盘
            if flag_mount==0:#未挂载
                ...
                HeartBeat['SD_TC'] = lines.split(" ")[1]
                HeartBeat['SD_Used'] = lines.split(" ")[2]
        else:#未插入
...
def dingshi():
    while True:
        resetNet()
        # logger.info(HeartBeat['Upload_mode'])
        time.sleep(1)
...
def takeOff():
    ...
    if wait_for_ack(1):  # 解锁命令等待应答成功
        ...
        if wait_for_ack(1):  # 开始任务命令等待应答成功
            ...
            logger.info("起飞ing")
            iot_reply_command(1, 0, "起飞执行成功，当前任务号"+HeartBeat['mission_id'])
        else:
            iot_reply_command(1, 1, "解锁成功-起飞被拒绝")
    else:
        iot_reply_command(1, 1, "解锁被拒绝")

def on_topic_message(topic, payload, qos, userdata):#iotget
    # 接受阿里云iot json数据包
    ...
    if jieshou['Type'] == 'Command':
        if jieshou['CMD'] == 1:  # 起飞指令
            #起飞前检查
            if HeartBeat['Upload_mode']==1:
                takeOff()
            elif HeartBeat['Upload_mode']==2 or HeartBeat['Upload_mode']==3:
                if HeartBeat['SD'].isdigit():#SD插入了
                    takeOff()
                else:#SD卡未插入
                    iot_reply_command(1,1,"照片存储媒介未插入，请检查")
            elif HeartBeat['Upload_mode']==3:
                logger.info("视频模式")
        ...
        try:
            ...
            get_photoNum(mission_num)
            #参数配置检查
            if 'Upload_mode' not in jieshou:
                raise Exception("APP版本未更新")
            if int(jieshou['Upload_mode'])==2:
                if int(jieshou['ShutterTime'])<5:#参数大于5
                    raise Exception("参数错误")
            elif int(jieshou['Upload_mode'])==3:
                if int(jieshou['Distance'])<100:
                    raise Exception("参数错误")
            elif int(jieshou['Upload_mode']==4):
                raise Exception("当前硬件不支持视频模式")
            HeartBeat['Upload_mode']=int(jieshou['Upload_mode'])
            HeartBeat['ShutterTime']=int(jieshou['ShutterTime'])
            HeartBeat['Distance']=int(jieshou['Distance'])
            with open('initial.ini', 'r') as f:
                write_json=f.read()
            writedata = json.loads(write_json)  # 将json转换为字典
            writedata['Upload_mode'] = HeartBeat['Upload_mode']
            writedata['ShutterTime'] = HeartBeat['ShutterTime']
            writedata['Distance'] = HeartBeat['Distance']
            write_json = json.dumps(writedata)
            with open('initial.ini', 'w') as f:
                f.write(write_json)
            logger.info("参数写入成功")
        ...
        try:
            logger.info(jieshou)
            if jieshou['action']=='disarm':
                aa.command_long_send(the_connect.target_system, the_connect.target_component, 31014,
                                     0, 1, 0, 0, 0, 0, 0, 0)  # 开始任务
                # the_connect.arducopter_arm()
                logger.info("动力检查开始")
                if wait_for_ack(1):
                    iot_reply_ok('arm_test')
                else:
                    iot_reply_other('arm_test','飞控拒绝，请检查飞控')
            elif jieshou['action']=='arm':
                logger.info("动力检查结束")
                aa.command_long_send(the_connect.target_system, the_connect.target_component, 31014,
                                     0, 0, 0, 0, 0, 0, 0, 0)  # 开始任务
    ...
    elif jieshou['Type']=='rtmp':
        try:
            HeartBeat['rtmp_url']=jieshou['url']
            with open('initial.ini', 'r') as f:
                write_json = f.read()
            writedata = json.loads(write_json)  # 将json转换为字典
            writedata['rtmp_url'] = HeartBeat['rtmp_url']
            write_json = json.dumps(writedata)
            with open('initial.ini', 'w') as f:
                f.write(write_json)
            logger.info("rtmp url保存成功")
            #开启rtmp推流线程
            iot_reply_ok('rtmp')
        except Exception as e:
            iot_reply_other('rtmp',e)
    elif jieshou['Type'] == 'set_geofence':
        try:
            the_connect.mav.param_set_send(the_connect.target_system,
                                           the_connect.target_component,
                                           "GF_ACTION".encode('utf8'),
                                           0,
                                           cm.MAV_PARAM_TYPE_INT32)
            # aa.command_long_send(the_connect.target_system, the_connect.target_component, cm.MAV_CMD_DO_FENCE_ENABLE,
            #                      0, 0, 0, 0, 0, 0, 0, 0)  # 开始任务
            if wait_for_ack(0.2):
                iot_reply_ok("set_geofence")
            else:
                iot_reply_error("set_geofence","解除失败，请检查飞控")
        except Exception as e:
            iot_reply_error("set_geofence",str(e))
    else:
        logger.info(jieshou)
...
def dingshi_paiZhao():
    ...
    while flag_start == 1:
        try:
            ...
            photoNum = photoNum + 1
            HeartBeat['PhotoNum']=photoNum
            if HeartBeat['Upload_mode']==2:#等时拍照
                time.sleep(HeartBeat['ShutterTime'])
            elif HeartBeat['Upload_mode']==3:#等距拍照
                #时间=设置的距离/当前地速
                if HeartBeat['groundspeed']>15:#刚起飞时
                    delayTime = HeartBeat['Distance']/HeartBeat['groundspeed']
                    logger.info("当前地速为" + str(HeartBeat['groundspeed']))
                    logger.info("计算延迟时间为" + str(delayTime))
                    if delayTime < 5:
                        time.sleep(5)
                    else:
                        time.sleep(delayTime)
                else:
                    logger.info("地速过低为"+str(HeartBeat['groundspeed']))
                    time.sleep(5)
            camera.exit()
        except Exception as e:
...
if __name__ == "__main__":
    ...
    while True:
        if flag_start == 1:
            ...
            t5.start()
            if HeartBeat['Upload_mode']==1:#飞控模式
                pass#不启动任何其他线程
            elif HeartBeat['Upload_mode']==2:#等时拍照模式
                ...
                t3.join()
                t4.join()
            elif HeartBeat['Upload_mode']==3:#等距拍照模式
                t3 = threading.Thread(target=dingshi_paiZhao)
                t3.start()
                t4 = threading.Thread(target=post_image)
                t4.start()
                t3.join()
                t4.join()
            elif HeartBeat['Upload_mode']==4:#视频模式
                pass#在收到命令的时候已经开启新的线程完成视频推流
            t5.join()
        time.sleep(1)
    t1.join()
    t2.join()
