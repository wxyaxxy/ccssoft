...
def jiexi_Aoshi(file):
    ...
    for i in range(len(Takeoff_dict["TakeOff"])):
        ...
        mis_dict["x"] = int(Takeoff_dict["TakeOff"][i]["Latitude"]* 10000000)
        mis_dict["y"] = int(Takeoff_dict["TakeOff"][i]["Longitude"]* 10000000)
    ...
    for i in range(len(plan_dict["mission1"])):
        ...
        mis_dict["x"] = int(plan_dict["mission1"][i]["Latitude"]* 10000000)
        mis_dict["y"] = int(plan_dict["mission1"][i]["Longitude"]* 10000000)
    ...
    for i in range(len(land_dict["Land"])):
        ...
        mis_dict["x"] = int(land_dict["Land"][i]["Latitude"]* 10000000)
        mis_dict["y"] = int(land_dict["Land"][i]["Longitude"]* 10000000)
...
def xiaFangHangXian():
    ...
    if wait_for_ack(0.5):
        mis_dict = jiexi_Aoshi("planfile/"+planFileName)
    ...
    while True:
        try:
            ...
            try:
                ...
                HeartBeat['voltage_battery']=dianya
                ...
                if HeartBeat['battery_remaining'] <0:
                    HeartBeat['battery_remaining'] = 100
            except:
                pass
            try:
...
def dingshi():
    while True:
        resetNet()
        # logger.info(HeartBeat['voltage_battery'])
        time.sleep(1)
            ...
            if xiaFangHangXian():
                ...
                if read_planfile():
                    if plan_check():
                        ...
                        iot_reply_other('check_line', '核对错误，请重试')
                else:
                    iot_reply_other('check_line','读取航线错误，请重试')
            else:
